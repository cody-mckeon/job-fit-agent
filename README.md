...
The discovery commands accept `--source-file <path>`, `--query <text>`, `--location <text>`, and `--limit <n>`, then normalize CSV, JSON, JSONL, YAML, Markdown, or plain-text lists into `data/work_opportunities.json` with `fit_score`, `actionability_score`, `revenue_potential`, `relationship_value`, `urgency_score`, and `recommended_next_action`. Lane-specific qualification checks cover W2 role/geography/block/channel, contract business problem/cycle/AI fit/buyer/revenue/delivery risk, RFP deadline/eligibility/documents/scope/complexity/realistic submission, local workflow pain/relevance/decision-maker/pilot/agent-deployment potential, and relationship relevance/warmth/reach-out reason/opportunity path. Lane defaults keep contract/1099, fractional, vendor, and local leads in `qualify`, RFPs in `proposal_needed` when a deadline is present, and relationship leads in `relationship_strategy`.

`work-opportunities` groups durable records into Pursue now, Proposal needed, Relationship strategy, Research / qualify, Blocked, Submitted / waiting, Won, and Lost / skipped. The prep commands create markdown folders under `applications/work_opportunities/`: `prep-rfp` writes `rfp_summary.md`, `go_no_go_checklist.md`, `required_documents.md`, `proposal_outline.md`, `questions_to_ask.md`, `risks.md`, and `next_steps.md`; `prep-1099` writes `opportunity_brief.md`, `qualification_checklist.md`, `scope_hypothesis.md`, `pricing_hypothesis.md`, `outreach_note.md`, `risks.md`, and `next_steps.md`; `prep-local-outreach` writes `business_profile.md`, `pain_hypothesis.md`, `ai_pilot_idea.md`, `diagnostic_offer.md`, `outreach_note.md`, and `follow_up_sequence.md`. Company-level W2 blocks suppress direct W2 recommendations, but they do not automatically block separate RFP, vendor, 1099, or local-business strategies for the same organization, and weak W2 jobs are not allowed to beat strong contract/RFP/local opportunities.
