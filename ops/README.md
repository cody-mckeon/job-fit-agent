...
If a job exists online but is missing from local SQLite, prepare directly from the Ashby or Workday job page with `prep-url`. This path is useful when an Ashby board API collector returns `403` but the direct posting still renders:
...
docker compose run --rm job-fit-agent python -m job_fit_agent.main rescore-work-opportunities
...
Use this layer for all work Cody could pursue, not only W2 roles: 1099 contracts, fractional roles, RFPs, vendor opportunities, local business opportunities, relationship opportunities, and manual leads from podcasts, LinkedIn, local events, or conversations. `prep-rfp` produces decision-grade summaries, go/no-go factors, unknown-required-document checklists, procurement/data-access questions, risk analysis, and ordered next steps from the opportunity record. `prep-local-outreach` produces usable local business-development artifacts from the same durable record: target profile, pain hypotheses, small pilot ideas, diagnostic offer, sendable note, follow-up sequence, and risk mitigations. Discovery commands support `--source-file`, `--query`, `--location`, and `--limit`; source files can be CSV, JSON, JSONL, YAML, Markdown, or plain text. Records are durable in `data/work_opportunities.json` with fit, actionability, urgency, revenue, relationship, deadline/blocking, qualification, risk, and recommended-next-action fields. Run `rescore-work-opportunities` after editing records or upgrading scoring rules to recompute derived fields and refresh `updated_at` without requiring every local-business lead to have a URL. Lane defaults keep contract/1099 leads in qualification, RFPs in go/no-go or proposal prep, local businesses in diagnostic research, and relationships in outreach strategy. `opportunity-review` compares W2 jobs, company-universe records, Opportunity Pipeline strategy, Work Opportunity Engine records, blocked companies, deadlines, relationship value, urgency, actionability, revenue potential, and fit score, then recommends the highest-leverage next action today. A W2 company cooldown does not automatically prevent separate 1099, RFP, vendor, or local-business strategy for that organization, and weak W2 jobs should not beat strong contract/RFP/local opportunities.
...
Application prep uses role-family project selection for generated resume strategy, draft, cover letter, and recruiter note. AI Enablement Manager, AI Transformation, AI Operations, AI Implementation, AI Solutions Engineer, Product Operations AI, and Marketing Operations AI roles prioritize Marketing Intelligence OS near the top of the project section; limited-space versions should use its top three bullets for intake/use-case/reporting workflow structure, repeatable campaign/website/UTM/CTA/booking-intent analysis workflows, and internal AI enablement through real workflow improvements. Enterprise solutions, forward deployed, workflow automation, and internal tools roles otherwise prioritize AI Product Design Operating System, Job Fit Agent, and RWLV Priority Governor Agent; product manager roles prioritize AI Product Design Operating System, RWLV Priority Governor Agent, and Job Fit Agent; Lennar Product Manager / Digital Buying & Selling roles prioritize Resorts World digital product and analytics work, AI Marketing Intelligence Platform display naming, product discovery, requirements, user behavior, conversion optimization, GA4/GTM/Pendo instrumentation, customer journey, and AI-assisted product lifecycle language; analytics/product systems roles prioritize RWLV Priority Governor Agent, AI Product Design Operating System, and Job Fit Agent. Summary copy should use concrete AI-enabled workflow systems language rather than buzzword-heavy AI-native defaults.
